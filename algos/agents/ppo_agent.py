...
Actor_PATH = '../../cgames/06_sonic2/checkpoints/ppo_actor'
Critic_PATH = '../../cgames/06_sonic2/checkpoints/ppo_critics'

class PPOAgent():
    def __init__(self, input_shape, action_size, seed, device, gamma, alpha, beta, tau, update_every, batch_size, ppo_epoch, clip_param, actor_m, critic_m, mode):
        ...
        if mode == 0:
            ...
            # self.actor_net, self.critic_net = self.load_state_dict()
    ...
    def save_models(self):
        self.version += 1
        torch.save(self.actor_net.state_dict(), Actor_PATH+str(self.version)+'.pth')
        torch.save(self.critic_net.state_dict(), Critic_PATH+str(self.version)+'.pth')

    def load_state_dict(self):
        # actor_net = self.actor_m(self.input_shape, self.action_size).to(self.device)
        # critic_net = self.critic_m(self.input_shape).to(self.device)

        self.actor_net.load_state_dict(torch.load(Actor_PATH+str(self.version)+'.pth'))
        self.critic_net.load_state_dict(torch.load(Critic_PATH+str(self.version)+'.pth'))

        # actor_net_model_state = actor_net.load_state_dict(torch.load(Actor_PATH+str(197)+'.pth', map_location='cpu'))
        # critic_net_model_state = critic_net.load_state_dict(torch.load(Critic_PATH+str(197)+'.pth', map_location='cpu'))
        # return actor_net_model_state, critic_net_model_state

    def load_models(self):
        actor_net = self.actor_m(self.input_shape, self.action_size).to(self.device)
        critic_net = self.critic_m(self.input_shape).to(self.device)
        actor_net_model_state = torch.load(Actor_PATH, map_location=self.device)
        critic_net_model_state = torch.load(Critic_PATH, map_location=self.device)
        return actor_net_model_state, critic_net_model_state

    def save_check_point(self):
        torch.save({
            'epoch': self.ppo_epoch,
            'model_state_dict': self.actor_net.state_dict(),
            'optimizer_state_dict': self.actor_optimizer.state_dict(),
            }, Actor_PATH+str(self.version)+'.pth')

        torch.save({
            'epoch': self.ppo_epoch,
            'model_state_dict': self.critic_net.state_dict(),
            'optimizer_state_dict': self.critic_optimizer.state_dict(),
            }, Critic_PATH+str(self.version)+'.pth')

    def load_check_point(self):
        print("Load Checkpoints")
        actor_ckp = torch.load(Actor_PATH+str(self.version)+'.pth')
        critic_ckp = torch.load(Critic_PATH+str(self.version)+'.pth')
        self.actor_net.load_state_dict(actor_ckp["model_state_dict"])
        self.critic_net.load_state_dict(critic_ckp["model_state_dict"])